...
//                    showProgressBar()
                ...
                is State.Failed -> {
                    val bundleTracking =
                        bundleOf("EventEdit Error [assignParticipants]" to state.message)
...
//                    showProgressBar()
                ...
                is State.Failed -> {
                    val bundleTracking = bundleOf("EventEdit Error [assignClub]" to state.message)
                    Constants.TRACKING.logEvent(Tracking.ALERT_ERROR, bundleTracking)
    ...
    fun setEventDate(eventDay1: String, eventDay2: String, eventTime: String): Date? {
        ...
        try {
            ...
            val bundleTracking = bundleOf("EventEdit Error [setEventDate]" to e.localizedMessage)
            Constants.TRACKING.logEvent(Tracking.ALERT_ERROR, bundleTracking)
